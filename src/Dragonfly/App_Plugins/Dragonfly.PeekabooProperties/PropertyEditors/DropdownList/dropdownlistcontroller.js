...
function ($scope, $q, $timeout, assetsService) {
    ...
    if ($scope.model.config.useChosen == 1) {
        ...
        if (typeof chosen === "undefined") { // Don't reload the chosen library if it is already loaded
            await.push(assetsService.loadJs('/App_Plugins/Dragonfly.PeekabooProperties/ThirdParty/chosen/chosen.jquery.min.js', $scope));
            await.push(assetsService.loadCss('/App_Plugins/Dragonfly.PeekabooProperties/ThirdParty/chosen/chosen.min.css', $scope));
        }
    }

    await.push(assetsService.loadJs('/App_Plugins/Dragonfly.PeekabooProperties/PropertyEditors/common.js', $scope));
